---
name: doc2md
description: Конвертирует Word, PowerPoint, Excel, OpenDocument, RTF, EPUB, CSV и PDF в чистый Markdown ДО того, как агент начинает читать документ — один файл, список файлов или целая папка разом. Используй, когда нужно разобрать пачку разноформатных документов (договоры, акты, презентации, выгрузки), когда штатный ридер под формат либо отсутствует, либо избыточен для простой пачки, или когда документов больше 2-3 и дешевле прогнать всё разом, чем открывать по одному. Триггеры — "сконвертируй эти файлы", "разбери пачку документов", "не могу прочитать этот файл", "переведи в markdown", "подготовь документы для агента".
license: MIT
metadata:
  author: ilyautov
  version: 1.2.0
